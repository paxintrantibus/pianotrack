/* PianoTrack native module. Standalone v2; storage is isolated from v1. */
window.PianoModule=(() => {
  const STORAGE_KEY='piano-practice-timer-v2', ACTIVE_KEY='piano-practice-active-v2', RECORD_MODE_KEY='piano-practice-record-mode-v2', PREVIOUS_KEY='piano-practice-previous-v2', GOAL_KEY='piano-practice-goal-v2';
  const APP_VERSION='2.0', SCHEMA_VERSION=1;
  ...
  function applyImport(){if(!pendingImport)return;sessions=pendingImport;pendingImport=null;saveSessions();activeStart=null;localStorage.removeItem(ACTIVE_KEY);importDialog.close();renderAll();showToast('バックアップを復元しました')}
  function restorePreviousData(){const raw=localStorage.getItem(PREVIOUS_KEY);if(raw===null)return;try{const restored=normalizeSessions(parseStoredData(raw).sessions),current=localStorage.getItem(STORAGE_KEY);sessions=restored;localStorage.setItem(STORAGE_KEY,JSON.stringify(makeStoredData()));if(current!==null)localStorage.setItem(PREVIOUS_KEY,current);restoreDialog.close();renderAll();showToast('直前の状態に戻しました')}catch{showToast('直前のデータを復元できませんでした')}}
  function resetAllData(){sessions=[];saveSessions();activeStart=null;localStorage.removeItem(ACTIVE_KEY);resetFinalDialog.close();renderAll();showToast('すべての記録を削除しました')}
  ...
  function renderAll(){renderTimer();renderSummary();renderRecords();renderDataInfo();renderGoal()}

  function startTimer(){activeStart=Date.now();localStorage.setItem(ACTIVE_KEY,String(activeStart));renderAll();showToast('練習を開始しました')}
  function requestFinish(){if(!activeStart)return;sessionResult.textContent=formatExactDuration(Date.now()-activeStart);finishDialog.showModal()}
  function finishAndSave(){if(!activeStart)return;const endedAt=Date.now(),durationMs=endedAt-activeStart;if(durationMs>=1000){sessions.push({id:makeId(),startedAt:activeStart,endedAt,durationMs,note:''});saveSessions()}activeStart=null;localStorage.removeItem(ACTIVE_KEY);finishDialog.close();selectedCalendarDay=localDateKey(endedAt);renderAll();showToast('練習時間を保存しました')}
  ...
  setInterval(()=>{
    if(activeStart && document.body.classList.contains('piano-native-mode')){
      renderTimer();
      renderSummary();
  ...
  return {
    ...
    open(){
      document.body.classList.remove('launcher-mode','soup-native-mode');
      document.body.classList.add('piano-native-mode');
    ...
    close(){
      ...
      root.classList.remove('timer-view');
    },

    hasTodayRecord(date=localDateKey(Date.now())){
      return rowsForDay(date).length>0;
    },

    getDiaryEntries(date=localDateKey(Date.now())){
      const rows=rowsForDay(date);
    ...
    getBackupData(){return{sessions:structuredClone(sessions),goal:loadGoal(),recordMode};},
    getDefaultBackupData(){return{sessions:[],goal:{type:'daily',minutes:30},recordMode:'calendar'};},
    validateBackupData(data){
      if(!data||!Array.isArray(data.sessions))throw new Error('ピアノデータの形式が不正です。');
      normalizeSessions(data.sessions);
      if(!data.goal||!['daily','monthly','yearly'].includes(data.goal.type)||!Number.isInteger(data.goal.minutes)||data.goal.minutes<=0)throw new Error('ピアノ目標の形式が不正です。');
      if(!['calendar','list'].includes(data.recordMode))throw new Error('ピアノ表示設定の形式が不正です。');
    },
    restoreBackupData(data){this.validateBackupData(data);sessions=normalizeSessions(data.sessions);localStorage.setItem(GOAL_KEY,JSON.stringify(data.goal));recordMode=data.recordMode;localStorage.setItem(RECORD_MODE_KEY,recordMode);saveSessions();activeStart=null;localStorage.removeItem(ACTIVE_KEY);renderAll();},
